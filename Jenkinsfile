pipeline {
    ...
    stages {
        ...
        stage('Run Branch Comparison') {
            steps {
                script {
                    // Write the Python script directly in the Jenkinsfile
                    writeFile file: 'branch_comparison.py', text: '''
import requests
import os
import sys
...
def display_repository_results(results, source, requested_destination):
    ...
    summary = (
        ...
        f"Repositories with errors: {len(repos_with_errors)}\\n"
        f"Repositories with fallback branch: {len(repos_with_alt_branch)}\\n"
        f"Repositories without changes: {len(repos_without_changes)}"
    )
